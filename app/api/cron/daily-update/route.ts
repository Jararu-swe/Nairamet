...
/**
 * Cron job that re-scrapes articles daily
 * Runs once daily at 6 AM UTC
 * Articles are cached in memory for 24 hours, then re-scraped
 */
export async function GET(request: Request) {
  ...
  // 1. Fetch articles (will re-scrape if cache expired)
  try {
    ...
    console.log(
      "[Cron] Articles re-scraped successfully:",
