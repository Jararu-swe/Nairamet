...
import { format, subDays, subYears, isWithinInterval, parseISO } from "date-fns"
...
const generateHistoricalData = () => {
  ...
  for (let i = 365; i >= 0; i--) {
    ...
    currencies.forEach((currency) => {
      const baseRate = currency === "USD" ? 1580 : currency === "GBP" ? 1950 : currency === "EUR" ? 1720 : 230
...
function LogsPageContent() {
  ...
  const filteredData = useMemo(() => {
    ...
    filtered = filtered.filter((item) => isWithinInterval(parseISO(item.timestamp), { start: startDate, end: today }))
  ...
  const statistics = useMemo(() => {
    ...
    const stats = currencies
      .map((currency) => {
        const currencyData = filteredData.filter((item) => item.currency === currency)
        if (currencyData.length === 0) return null

        const officialRates = currencyData.map((item) => item.officialRate)
        const blackMarketRates = currencyData.map((item) => item.blackMarketRate)

        const avgOfficial = officialRates.reduce((a, b) => a + b, 0) / officialRates.length
        const avgBlackMarket = blackMarketRates.reduce((a, b) => a + b, 0) / blackMarketRates.length
  ...
  const exportToCSV = () => {
    const headers = ["Date", "Currency", "Official Rate", "Black Market Rate", "Spread"]
    const csvContent = [
      headers.join(","),
      ...filteredData.map((item) =>
        [item.date, item.currency, item.officialRate, item.blackMarketRate, item.spread].join(","),
      ),
    ].join("\n")
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-emerald-900">Rate Logs & Archive</h1>
          <p className="text-emerald-700">Search historical rates and export data</p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Find specific rates and filter by date range or currency</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Complete archive of exchange rates with official and black market data</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right p-2 font-medium">Official Rate</th>
                  ...
                  {filteredData.slice(0, 100).map((item, index) => {
                    ...
                    return (
                      <tr key={item.id} className="border-b hover:bg-muted/50">
                        <td className="p-2 text-sm">{format(parseISO(item.timestamp), "MMM dd, yyyy")}</td>
                        <td className="p-2">
                          <Badge variant="outline">{item.currency}/NGN</Badge>
                        </td>
                        <td className="p-2 text-right font-mono">₦{item.officialRate}</td>
                        <td className="p-2 text-right font-mono">₦{item.blackMarketRate}</td>
                        <td className="p-2 text-right font-mono text-orange-600">₦{item.spread}</td>
                        <td className="p-2 text-center">
                          {trend === "up" && <TrendingUp className="w-4 h-4 text-red-500 mx-auto" />}
                          {trend === "down" && <TrendingDown className="w-4 h-4 text-green-500 mx-auto" />}
                          {trend === "same" && <div className="w-4 h-4 mx-auto" />}
                        </td>
                      </tr>
                    )
                  })}
              ...
              {filteredData.length > 100 && (
                <div className="text-center py-4 text-muted-foreground">
                  Showing first 100 of {filteredData.length} records. Use filters to narrow results or export for
                  complete data.
                </div>
              )}
