...
export class UserStorage {
  ...
  // User Management
  static saveUser(user: { email: string; name: string; tier?: "free" | "premium"; trialEndsAt?: Date }): UserData {
  ...
  static saveAlert(alert: Omit<UserAlert, "id" | "userId" | "createdAt">): UserAlert {
    const user = this.getUser()
  ...
  static updateAlert(alertId: string, updates: Partial<UserAlert>): UserAlert | null {
  ...
  static getUserStats() {
    ...
    return {
      ...
      notificationsEnabled: Object.values(preferences.notifications).some(Boolean),
    }
  }
  ...
  static canCreateAlert(): { allowed: boolean; reason?: string } {
    ...
    const alerts = this.getUserAlerts()
    const isPremium = user.tier === "premium" || (user.trialEndsAt && new Date() < new Date(user.trialEndsAt))

    if (!isPremium && alerts.length >= 1) {
      return { allowed: false, reason: "Free users can only create 1 alert. Upgrade to Premium for unlimited alerts." }
    }

    return { allowed: true }
  }

  static canAccessHistoricalData(days: number): { allowed: boolean; reason?: string } {
    const user = this.getUser()
    if (!user) return { allowed: false, reason: "User not logged in" }
    ...
    if (!isPremium && days > 7) {
      return {
        allowed: false,
        reason: "Free users can only view last 7 days. Upgrade to Premium for full historical data.",
      }
    }
  ...
  static canAccessRateLogs(): { allowed: boolean; reason?: string } {
    ...
    if (!isPremium) {
      return {
        allowed: false,
        reason: "Rate logs are a Premium feature. Upgrade to access searchable historical data.",
      }
    }
  ...
  static canExportData(): { allowed: boolean; reason?: string } {
    ...
    if (!isPremium) {
      return { allowed: false, reason: "Data export is a Premium feature. Upgrade to download charts and export data." }
    }
